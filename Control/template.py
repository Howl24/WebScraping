...
class Template:
    def __init__(self, job_center, func_filename, url_base, period, area_url,
                 areas_source, num_offSource, links_per_page, num_sources,
                 list_sources, date_feature, first_level_sources):
    ...
    def read_attributes_from_file(file, main_list):
        ...
        if num_sources is None:
            ...
            for i in range(0, num_sources):
                ...
                if offSource is None:
                    main_list.add_msg("Failed to read the offer source #" + str(i+1), MessageList.ERR)
                else:
                    list_sources.append(offSource)
        ...
        for i in range(0, num_first_features):
            ...
            if features is None:
                if msg_list.contains_errors():
                    msg_list.set_title("Failed to read first level features Source #{num}".format(num=i),
                                       MessageList.ERR)
        ...
        if main_list.contains_errors():
            ...
            return job_center, func_filename, url_base, period, area_url, areas_source, num_offSource, \
                   links_per_page, num_sources, list_sources, date_feature, first_level_sources
            ...
            if offer is not None:
                if dates is not None:
                    ...
                    try:
                        ...
                        main_list.set_title("Can't find '{feature}' field to get publication date".
                                            format(feature=self.date_feature), MessageList.ERR)
                        return None
                # check!
                try:
    ...
    def read_attributes_from_file(file, main_list):
        ...
        while True:
            ...
            if features_source is None:
                if msg_list.contains_errors():
                    msg_list.set_title("Failed to read features Source #"
                                       + str(len(features_sources)+1), MessageList.ERR)
                    main_list.add_msg_list(msg_list)
